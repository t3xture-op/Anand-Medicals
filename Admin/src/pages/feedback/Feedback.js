...
const Feedback= () => {
    ...
    const handleAuthError = (err) => {
        ...
        if (errorMessage === 'Unauthorized' || errorMessage.includes('jwt expired')) {
    ...
    const handleStatusUpdate = async (id, status) => {
        try {
            ...
            toast.success(`Feedback ${status ? 'approved' : 'declined'} successfully!`);
            fetchFeedbacks();
        } catch (err) {
    ...
    if (feedbacks.length === 0) return <div className="text-center py-8 text-gray-500">No feedback submitted yet.</div>;
                        ...
                        {feedbacks.map((feedback) => (
                            <tr key={feedback._id}>
                                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p className="text-gray-900 whitespace-no-wrap">{feedback.name}</p>
                                </td>
                                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p className="text-gray-900 whitespace-no-wrap">{feedback.email}</p>
                                </td>
                                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p className="text-gray-900 whitespace-normal">{feedback.message}</p>
                                </td>
                                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <div className="flex">
                                        {(() => {
                                            console.log('Feedback Rating received:', feedback.rating);
                                            const rating = Number.isFinite(feedback.rating) && feedback.rating >= 0 && feedback.rating <= 5
                                            ...
                                            return (
                                                <>
                                                    ...
                                                    {[...Array(emptyStars)].map((_, i) => (
                                                        <Star key={i + filledStars} className="w-5 h-5 text-gray-300" />
                                                    ))}
                                                </>
                                            );
                                        })()}</div>
                                ...
                                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <span
                                        className={`relative inline-block px-3 py-1 font-semibold leading-tight ${
                                            feedback.approved ? 'text-green-900' : 'text-red-900'
                                        }`}
                                    >
                                        <span
                                            aria-hidden="true"
                                            className={`absolute inset-0 opacity-50 rounded-full ${
                                                feedback.approved ? 'bg-green-200' : 'bg-red-200'
                                            }`}
                                        ></span>
                                        <span className="relative">{feedback.approved ? 'Approved' : 'Pending'}</span>
                                    </span>
                                </td>
