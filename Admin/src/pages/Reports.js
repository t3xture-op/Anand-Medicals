...
import {
  ...
  BarChart2, TrendingUp, Package, Users
} from 'lucide-react';
...
export default function Reports() {
  ...
  useEffect(() => { fetchStats(); fetchDetails(); }, [reportType]);
    ...
    <div className="space-y-6 fade-in">
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="mt-1">
              <select id="reportType" value={reportType} onChange={e=>{
                  setReportType(e.target.value);
                  setDetailsData([]);
              }}
                className="block w-full border border-gray-300 rounded-md py-2 pl-3 pr-10 text-base focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
          ...
          <div className="flex-1">
            ...
            <div className="mt-1 flex items-center">
              ...
              <input type="date" value={dateRange.to} min={dateRange.from}
                ...
                className="w-full border rounded-md px-2 py-1 text-sm"/>
              <button onClick={()=>{fetchStats(); fetchDetails()}}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {['sales','orders','products','users'].map(type=> {
          const Icon = {'sales':BarChart2,'orders':Calendar,'products':Package,'users':Users}[type];
          const colors = {'sales':'blue','orders':'purple','products':'green','users':'amber'}[type];
          const titles = {'sales':'Total Sales','orders':'Total Orders','products':'Total Products','users':'Total Users'}[type];
          const growth = {'sales':'+18.3%','orders':'+12.5%','products':'+5.2%','users':'+9.7%'}[type];
          const vals = {
          ...
          return (
            <div key={type} className="card">
              <div className="flex justify-between items-center">
                <h2 className="text-sm font-medium text-gray-500">{titles[type]}</h2>
                <div className={`rounded-full bg-${colors}-50 p-2 text-${colors}-600`}>
                  <Icon size={20}/>
                </div>
              </div>
              <p className="mt-2 text-2xl font-semibold text-gray-800">
                {type==='sales' ? `₹${vals[type]?.toLocaleString()}` : vals[type] || 0}
              </p>
              <div className="mt-2 flex items-center">
                <TrendingUp size={16} className="text-green-500"/>
                <span className="ml-1 text-xs font-medium text-green-600">{growth}</span>
      ...
      <div className="card">
        ...
        <div className="h-80">
          {!labels?.length || !data?.length ? (
            <div className="text-center text-sm text-gray-500">No data available for this report.</div>
          ) : (
            <DashboardChart
              type={type}
              labels={labels}
      ...
      {reportType === 'sales' && (
        <div className="card">
          <h2 className="mb-4 text-lg font-medium text-gray-800">Sales Details</h2>
      ...
      {reportType === 'orders' && (
        <div className="card">
          <h2 className="mb-4 text-lg font-medium text-gray-800">Orders Report</h2>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-50">
                <tr>
      ...
      {reportType === 'products' && (
        <div className="card">
          <h2 className="mb-4 text-lg font-medium text-gray-800">Product Performance</h2>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-50">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {detailsData.map((d, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="px-4 py-2 font-medium">₹{d.totalRevenue?.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
