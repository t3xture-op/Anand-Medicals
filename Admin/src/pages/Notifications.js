...
const Notifications = () => {
 ...
 const filteredNotifications = notifications.filter((n) => {
  const normalizedType = n.type.replace(" ", "_"); // "product request" → "product_request"
  return filter === "all" ? true : normalizedType === filter;
});
  ...
  const getNotificationIcon = (type) => {
    const icons = {
      ...
      product_request: <PackagePlus className="h-6 w-6 text-amber-500" />,
    };
  ...
  const clearAll = async () => {
    toast("DELETE PRODUCT", {
      description: "Do you want to clear all notifications?",
      action: {
        label: "DELETE",
        onClick: async () => {
          try {
